  ...
  if ( _LPSPI4 == this ) {
    if ( sniffer_enabled ) {
      if ( (_lpspi->CFGR1 & LPSPI_CFGR1_PINCFG(3)) != 0) { /* if pins are swapped */
        ...
        // IOMUXC_SW_MUX_CTL_PAD_GPIO_SD_B0_01 = 0x14; // LPSPI2_PCS0 (CS2) 36 ALT4 + SION. See: https://forum.pjrc.com/archive/index.php/t-59893.html
        // IOMUXC_SW_MUX_CTL_PAD_GPIO_SD_B0_00 = 0x14; // LPPI2_SCK1 (CLK2) 37 ALT4 + SION
        // IOMUXC_SW_MUX_CTL_PAD_GPIO_SD_B0_03 = 0x14; // LPSPI2_SDI (MISO2) 34 ALT4 + SION
        // IOMUXC_SW_MUX_CTL_PAD_GPIO_SD_B0_02 = 0x14; // LPSPI2_SDO (MOSI2) 35 ALT4 + SION
      }
    }
...
bool SPISlave_T4::available() {
  // Serial.print("RSR:");
  // Serial.print(_lpspi->RSR,BIN);
  // Serial.println("");
  // Serial.print("LPSPI_RSR_RXEMPTY:");
  // Serial.print(LPSPI_RSR_RXEMPTY, BIN);
  // Serial.println("");
  // Serial.print("available: ");
  // Serial.print(!(_lpspi->RSR & LPSPI_RSR_RXEMPTY));
  // Serial.println("");
...
uint32_t SPISlave_T4::popr() {
  ...
  // Serial.print(_lpspi->FSR, HEX);
  // Serial.print(" ");
  // Serial.print("RDR:");
  // Serial.print(_lpspi->RDR, BIN);
  // Serial.println("");
  return data;
}
...
void SPISlave_T4::SLAVE_ISR() {
  ...
  while ( available() ) {
    ...
    uint32_t val = popr();
    Serial.print("received: "); Serial.print(val, HEX);
  }
  Serial.println();
  _lpspi->SR = LPSPI_SR_DMF | LPSPI_SR_REF | LPSPI_SR_TEF | LPSPI_SR_TCF | LPSPI_SR_FCF | LPSPI_SR_WCF;
  asm volatile ("dsb");
