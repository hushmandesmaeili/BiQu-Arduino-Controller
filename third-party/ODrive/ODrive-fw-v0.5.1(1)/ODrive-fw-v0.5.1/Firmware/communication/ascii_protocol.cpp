...
void ASCII_protocol_process_line(const uint8_t* buffer, size_t len, StreamSink& response_channel) {
    ...
    if (cmd[0] == 'p') { // position control
        ...
        if (numscan < 2) {
            respond(response_channel, use_checksum, "p: invalid command format");
        } else if (motor_number >= AXIS_COUNT) {
            respond(response_channel, use_checksum, "p: invalid motor %u", motor_number);
        } else {
    ...
    } else if (cmd[0] == 'b') { // batch position control
        float pos_setpoint1, pos_setpoint2;
        int numscan = sscanf(cmd, "b %f %f", &pos_setpoint1, &pos_setpoint2);

        if (numscan < 1) {
            respond(response_channel, use_checksum, "b: invalid command format, numscan = %u", numscan);
        } else {
            Axis* axis0 = axes[0];
            Axis* axis1 = axes[1];
            axis0->controller_.config_.control_mode = Controller::CONTROL_MODE_POSITION_CONTROL;
            axis0->controller_.input_pos_ = pos_setpoint1;
            
            axis1->controller_.config_.control_mode = Controller::CONTROL_MODE_POSITION_CONTROL;
            axis1->controller_.input_pos_ = pos_setpoint2;

            axis0->controller_.input_pos_updated();
            axis1->controller_.input_pos_updated();
            
            axis0->watchdog_feed();
            axis1->watchdog_feed(); // unsure what watchdog_feed does and if this is appropriate
        }
        ...
        if (numscan < 2) {
            respond(response_channel, use_checksum, "q: invalid command format");
        } else if (motor_number >= AXIS_COUNT) {
            respond(response_channel, use_checksum, "q: invalid motor %u", motor_number);
        } else {
            Axis* axis = axes[motor_number];
            axis->controller_.config_.control_mode = Controller::CONTROL_MODE_POSITION_CONTROL;
            axis->controller_.input_pos_ = pos_setpoint;
        ...
        if (numscan < 2) {
            respond(response_channel, use_checksum, "v: invalid command format");
        } else if (motor_number >= AXIS_COUNT) {
            respond(response_channel, use_checksum, "v: invalid motor %u", motor_number);
        } else {
            Axis* axis = axes[motor_number];
        ...
        if (numscan < 2) {
            respond(response_channel, use_checksum, "c: invalid command format");
        } else if (motor_number >= AXIS_COUNT) {
            respond(response_channel, use_checksum, "c: invalid motor %u", motor_number);
        } else {
            Axis* axis = axes[motor_number];
        ...
        if (numscan < 1) {
            ...
            axis0->controller_.config_.control_mode = Controller::CONTROL_MODE_TORQUE_CONTROL;
            axis0->controller_.input_torque_ = torque_setpoint0;

            axis1->controller_.config_.control_mode = Controller::CONTROL_MODE_TORQUE_CONTROL;
            axis1->controller_.input_torque_ = torque_setpoint1;
    ...
    //     }
        ...
        if (numscan < 2) {
            // respond(response_channel, use_checksum, "invalid command format");
            respond(response_channel, use_checksum, "%f %f %f %f",
            ...
            respond(response_channel, use_checksum, "f: invalid motor %u", motor_number);
        } else {
            respond(response_channel, use_checksum, "%f %f",
                    (double)axes[motor_number]->encoder_.pos_estimate_,
                    (double)axes[motor_number]->encoder_.vel_estimate_);
